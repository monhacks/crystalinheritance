# -*- coding: utf-8 -*-
"""
Created on Wed Apr 23 13:57:20 2025

@author: dwg11
"""
"""
Pokémon TM/HM Compatibility Analyzer
...
Usage:
    python pokemon_tmhm_analyzer.py <constants_path> <base_stats_dir> [--output-dir <directory>]

Example:
    python pokemon_tmhm_analyzer.py constants/tmhm_constants.asm data/pokemon/base_stats --output-dir ./output
"""
...
def export_to_excel(compatibility_df, output_file='pokemon_tmhm_compatibility.xlsx'):
    """Export the compatibility matrix to an Excel file with formatting."""
    # Create a Pandas Excel writer
    writer = pd.ExcelWriter(output_file, engine='xlsxwriter')
    
    # Convert the dataframe to an XlsxWriter Excel object
    compatibility_df.to_excel(writer, sheet_name='TM_HM_Compatibility')
    
    # Get the workbook and worksheet objects
    workbook = writer.book
    worksheet = writer.sheets['TM_HM_Compatibility']
    
    # Define formats
    cell_format_yes = workbook.add_format({'bg_color': '#C6EFCE', 'font_color': '#006100'})
    cell_format_no = workbook.add_format({'bg_color': '#FFC7CE', 'font_color': '#9C0006'})
    
    # Apply conditional formatting
    for col_num, col in enumerate(compatibility_df.columns, 1):
        worksheet.conditional_format(1, col_num, len(compatibility_df.index), col_num, 
                                   {'type': 'cell',
                                    'criteria': '==',
                                    'value': 1,
                                    'format': cell_format_yes})
        worksheet.conditional_format(1, col_num, len(compatibility_df.index), col_num, 
                                   {'type': 'cell',
                                    'criteria': '==',
                                    'value': 0,
                                    'format': cell_format_no})
    
    # Adjust column widths
    worksheet.set_column(0, 0, 20)  # Pokémon name column
    worksheet.set_column(1, len(compatibility_df.columns), 12)  # Move columns
...
def main():
    ...
    if not pokemon_data:
        print("No Pokémon data found. Check your base_stats_dir path.")
        return
    ...
    excel_path = os.path.join(args.output_dir, 'pokemon_tmhm_compatibility.xlsx')
    ...
    export_to_excel(compatibility_df, excel_path)
